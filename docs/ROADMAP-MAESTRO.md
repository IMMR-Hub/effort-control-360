...
> 1. **La tarea 141 está `[~]` EN CURSO, en 5 ramas locales sin fusionar a
>    `main` ni pusheadas:** `tarea/141` (el importador, el servicio de IVA, la
>    pantalla y los bugs que encontró la auditoría — commits `0e9b2dd`,
>    `d43a78f`, `7a8f404`), `tarea/N6` (correos y alertas apagados por defecto —
>    `5303d4d`), `tarea/N2` (guardia de comandos versionada — `391ecda`),
>    `tarea/N4` (lista única de modelos del respaldo — `8516e7c`), `tarea/N1`
>    (carril de integración — `8f44267`). No reimplementar nada de esto ni
>    descartar esas ramas: `git branch` las lista, `git log --oneline <rama>`
>    muestra cada commit.
> 2. **Todavía no se puede subir.** Faltan, en este orden: (a) integrar las 5
>    ramas en una sola sobre `main` (tarea 8 del plan); (b) correr `npm run
>    verify` completo sobre esa integración (tarea 9); (c) la lectura de solo
>    lectura de los Excel reales de COPESA/FUMIPRO/ECOAGRO (N8 del plan, tarea
>    14 de la lista que le di a Daniel) — sin eso, la regla de "período
>    principal" de N7 y el tope de 12 meses de arrastre no están confirmados
>    contra los archivos de verdad, y la estimación de cuántas alertas/correos
>    generaría el primer cálculo automático sigue siendo una estimación, no una
>    medición; (d) tu autorización explícita para el push (A9).
> 3. **El orden propuesto, las autorizaciones y las preguntas para EFFORT**
...
ninguna otra tarea pendiente escondida en el archivo. Desde el 2026-09-17
puede haber una tarea `[~]` ("en curso ahora mismo, ver su nota de estado"):
si existe, va primero, aunque no sea `- [ ]`:
...
7. **Un push por vez a `main`**: DigitalOcean cancela el despliegue que está
   compilando si llega otro push. Esperar a que el sitio sirva la versión
   nueva antes de verificar — desde el 2026-09-17, con
   `node scripts/esperar-despliegue.mjs --marcador "..."` (confirma el bundle
   nuevo Y que la API responde; antes era "bajar el `.js` a mano", que sigue
   sirviendo si el script no está a mano).
8. **Ningún correo sale sin que Daniel lo autorice** (REGLA 0-bis de
   `CLAUDE.md`, 2026-09-16). Los avisos por correo quedan apagados por
...
**Avance: 104 de 114 tareas (91%).** Partes 1, 2, 3, 4A, 4B, 4C, 4D y 4E cerradas. App en producción en `https://effort360.disaak.com`, con dominio propio, HTTPS, variables de entorno, el equipo real completo (11 personas) y los 5 clientes piloto con 671 documentos reales cargados para el período del piloto (tareas 107-109, 56, 57). Cada persona ya puede dar de alta su propio segundo factor y cambiar su propia contraseña, que es lo que hace que la bitácora sirva como prueba de quién hizo qué (`docs/DISCREPANCIAS.md`, punto 16). Lo que sigue (tarea 111): que alguien del equipo entre de verdad y navegue las pantallas para confirmar que muestran estos datos reales — Claude no puede hacer ese último paso, no maneja contraseñas.

**Auditoría del 2026-09-10 (a pedido de Daniel, que vio todas las pantallas vacías).** El esqueleto está entero y probado (autenticación, permisos, bitácora, CRUD, 12 pantallas, RBAC sin `eliminar` para ningún rol), pero faltaba el motor: nada generaba vencimientos ni alertas. Encontrado y corregido en el día: el campo Período era texto libre que disparaba una petición por tecla (de ahí el error "Los datos enviados no son válidos" sin salida), ahora es selector de mes en las 5 pantallas que lo usan; y se construyó el **motor de vencimientos** (calendario tributario como dato, generador idempotente, 18 tests). También se construyó el **motor de alertas**, que antes no podía existir porque `RepositorioDeAlertas` no tenía método `crear`: ninguna línea del sistema podía levantar una alerta, y esa pantalla iba a estar vacía para siempre.
...
- [x] 57. Crear el primer usuario de dirección — **hecho el 2026-09-09/10.** Creado con `effort360@effort.com.py` (decisión de Daniel: no usar el correo de Laura ni el de Lili), rol `direccion`, `veTodosLosClientes`. Verificado con un ingreso real a `https://effort360.disaak.com`. La otra mitad de la tarea —"que la persona defina su propia contraseña en el primer acceso"— **no existía en el producto**: se construyó (ver `docs/DISCREPANCIAS.md`, punto 16, y la bitácora del 2026-09-10). **Resto del equipo sembrado el 2026-09-10:** 11 usuarios reales en total — `effort360`, Laura y Lilian con rol `direccion`; los otros 8 (Analia Sanguina, Aracely Alvarenga, Ariana Campos, Jorge Barrios, Karina Baez, Karina Medina, Melany González, Sandra Ferreira) con rol `responsable`, `veTodosLosClientes` para todos. Verificado contra la base real: 11 de 11, todos activos, ninguno con el segundo factor configurado todavía (lo hacen ellos mismos al entrar por primera vez). Credenciales entregadas a Daniel por archivo aparte, nunca en el chat, con instrucción de repartir cada una por un canal individual.
...
- [x] 115. **Caída total de producción del 2026-09-12 — causa encontrada y cerrada.** Daniel no podía entrar: "No se pudo establecer conexión segura con el servidor". El sitio cargaba (componente estático, sano) pero `GET /api/v1/csrf` devolvía `no_healthy_upstream (503 UH)` — DigitalOcean no tenía **ninguna** instancia viva de `api` a la que mandar la petición. El panel lo confirmaba: *"Containers repeatedly crashing"*, con CPU al 3% y RAM al 16%.
  ...
  **Cómo se identificó la causa sin un solo mensaje de error.** El log del contenedor terminaba en seco: arrancaba 11:06:57, atendía dos peticiones con 200, y a los ~3 minutos dejaba de existir. Tres ausencias lo definieron: (1) no apareció `Señal SIGTERM recibida`, que el propio código registra — así que DigitalOcean no lo reinició ordenadamente; (2) no se imprimió ninguna excepción — así que no fue un error de JavaScript; (3) no quedó absolutamente nada. La única muerte que no deja rastro es un `SIGKILL`, que no se puede interceptar, y el que lo manda en un contenedor es el kernel cuando se acaba la memoria.

  **Qué la causaba.** `sincronizarDesdeOneDrive` traía cada archivo **entero a memoria** (`leer()` → `Buffer`) sin mirar el tamaño, dentro de un contenedor de 512 MB, y arrancaba a los 2 minutos de cada despliegue. Un archivo suficientemente grande en la carpeta de EFFORT mataba el proceso; la corrida siguiente encontraba el mismo archivo y volvía a matarlo. Ciclo perfecto: el contenedor nunca vivía lo suficiente para que alguien entrara.

  **Cuatro arreglos, cada uno cerrando una parte distinta del agujero.** (1) `LIMITE_BYTES_POR_ARCHIVO = 25 MiB` en el sincronizador — lo que pasa de ahí no se descarga nunca y se anota como fallo con su tamaño, en vez de desaparecer. La comprobación va **antes** del `leer()`, y hay un test que falla si alguien la mueve después. (2) `TRABAJOS_AUTOMATICOS=si|no` en la configuración: apaga los trabajos de fondo desde el panel del hosting en dos minutos, sin desplegar código — durante este incidente la única forma de cortar el trabajo que mataba al servicio era un despliegue completo, con el sistema caído mientras tanto. (3) Manejadores de `unhandledRejection` y `uncaughtException` en `arrancar.ts`: no había ninguno, y esa ausencia fue lo más caro del incidente. Una promesa rechazada en un trabajo de fondo ya no puede tumbar la API entera — un aviso por correo que falla no puede dejar a EFFORT sin sistema. (4) `ESPERA_INICIAL` de 2 a 10 minutos y registro de memoria residente al empezar y terminar cada corrida, para que una muerte por memoria sea visible la próxima vez.
...
- **2026-09-09** — Tarea 107, causa raíz de ~15 builds fallidos seguidos en DigitalOcean, y una lección de método más cara que el bug. **Causa única:** el buildpack de Heroku lee `NODE_ENV=production` (cargada como variable con alcance de build) y le pasa a npm la configuración de producción, así que `npm ci` **no instalaba las devDependencies** — donde viven `typescript`, `@types/node` y el CLI de `prisma`, o sea todo lo que el build necesita. Un solo problema, tres síntomas que no se parecen entre sí y ninguno nombra la causa: errores de tipos (`Cannot find name 'process'`, `Buffer`, `node:crypto`), `sh: 1: prisma: not found` (exit 127), y builds colgados 30 minutos sin salida ni uso de CPU — este último era `npx tsc` intentando **descargar** typescript del registry, porque no estaba instalado. Corregido con una variable de entorno, `NPM_CONFIG_INCLUDE=dev` con alcance Build Time: en la resolución de configuración de npm `include` gana sobre `omit`/`production`, así que no hace falta tocar `NODE_ENV` — que sí se necesita en ejecución, porque de él depende el prefijo `__Host-` de la cookie de sesión. **Lo que costó caro:** esta hipótesis se había planteado en el primer build fallido y se descartó tras "verificarla"… en la máquina local, comprobando que npm suelto no omite devDependencies por `NODE_ENV`. Eso es cierto y es irrelevante: quien decide no es npm, es el buildpack. Verificar la hipótesis correcta en el entorno equivocado dio una falsa negativa y mandó a perseguir dos causas inventadas — "falta `@types/node` en `apps/api`" y "el `PATH` no resuelve `prisma`, hay que usar `npx`" — que arreglaban síntomas que no existían. Peor: el cambio a `npx` en el `postinstall` habría convertido un fallo rápido y explícito (`prisma: not found`) en un cuelgue silencioso descargando, o sea más de lo mismo que ya nos tenía trabados; se revirtió. **Lo que sí funcionó, y es el método a repetir:** reducir el `build_command` a solo `npm ci` para aislar (terminó bien en 2 minutos, descartando de una la teoría de "problema de red con npmjs.org"), y sobre todo **leer los números del log en vez de solo los mensajes de error** — "added 265 packages" y "42 packages are looking for funding" contra los 626 y 136 de la instalación local. Ese contraste era visible desde el primerísimo log fallido y decía la causa exacta. Regla para la próxima: cuando un entorno remoto falla y el local no, comparar primero **qué se instaló** en cada uno; y una hipótesis sobre el comportamiento de un entorno remoto solo se puede descartar con evidencia de ese entorno, nunca con una prueba local.
- **2026-09-09** — Tarea 107, segunda causa (independiente de la anterior, apareció recién cuando la primera dejó de tapar todo): **el CLI de `prisma` no se puede invocar desde la raíz del repo.** `npx prisma generate` en el `build_command` da `sh: 1: prisma: not found` (exit 127) aunque el paquete esté instalado — el comando corre desde la raíz, y `prisma` es devDependency del workspace `apps/api`, así que npm enlaza su binario en `apps/api/node_modules/.bin` y no en el `.bin` de la raíz, que es el único donde `npx` mira desde ahí. Comprobado mirando los dos directorios: en el de la raíz está `tsc` (typescript es devDependency de la raíz) y no está `prisma`; en el de `apps/api` está `prisma`. Y reproducido local: `npx prisma --version` desde la raíz falla, `npm exec --workspace @effort/api -- prisma --version` responde 6.19.3. Lo importante: **ese paso explícito nunca hizo falta.** El `postinstall` de `apps/api/package.json` ya genera el cliente y sí encuentra el binario, porque npm arma el PATH de los scripts de ciclo de vida con el `.bin` del propio workspace — el log lo prueba, `npm ci` termina en verde. El `npx prisma generate` lo había agregado yo como "segunda red de seguridad" varios intentos antes, y terminó siendo lo único que rompía el build. Corregido dejando `build_command: npm ci && npx tsc --build`. Lección: una redundancia "por las dudas" que nunca se probó no es gratis — acá fue la causa entera del fallo, y encima uno de los síntomas (`prisma: not found`) me hizo inventar una explicación equivocada sobre el `PATH` y cambiar el `postinstall` a `npx`, cambio que hubo que revertir. Antes de agregar un paso de refuerzo, verificar que ese paso funcione por sí solo en el entorno donde va a correr. Mismo bug latente corregido en el `run_command` del job `migrar-base` (todavía no creado en la app real).
- **2026-09-09** — Tarea 107, tercera y última causa: con el build ya en verde, el deploy quedó "Healthy" pero `GET /api/v1/csrf` daba 404 igual. DigitalOcean recorta el prefijo de ruta (`/api`) antes de reenviar la petición al contenedor, por defecto — pero las rutas de Fastify ya llevan el `/api` adentro del string (`app.get('/api/v1/csrf', ...)` en `servidor.ts`, no un prefijo separado registrado aparte). Sin avisarle a DigitalOcean que no recorte, `/api/v1/csrf` le llegaba a la app como `/v1/csrf`, no matcheaba ninguna ruta, y caía en el `setNotFoundHandler` propio — un 404 con forma de error nuestro, no de DigitalOcean, lo que lo hacía parecer un problema de código. Diagnosticado antes de tocar nada: pedir `/api/api/v1/csrf` (doble prefijo) daba `200 OK`, prueba directa de que faltaba un `/api`. Corregido en el panel activando **"Preserve Full Path"** en la regla de ruteo del componente (documentado en `.do/app.yaml` como `preserve_path_prefix: true`, aunque ese archivo no se sincroniza solo — ver nota de más arriba). **Verificado contra la app real, no asumido:** `GET https://effort-control-360-xiffl.ondigitalocean.app/api/v1/csrf` → `200`, con token CSRF real en la respuesta. Primera vez que el backend responde en producción de punta a punta — base de datos (Supabase), seguridad (Helmet/CSRF/rate-limit) y ruteo, los tres funcionando juntos. **Tarea 107 cerrada para el componente `api`.** Falta agregar el componente `web` (Static Site) — el 404 al pedir `/` es esperado, todavía no hay ningún componente sirviendo la raíz — y las tareas 108 (confirmar variables, hecho de facto), 109 (DNS) y 110 (HTTPS/cookies en el dominio propio).
- **2026-09-09** — Tarea 107 cerrada del todo: agregado el componente `web` (Static Site) en la misma app. Mismo patrón de auto-detect equivocado que la primera vez (proponía un "Web Service" de $24/mes + un "Function" espurio) — corregido a mano: Resource type → Static Site ($0/mes), Function eliminado, Build Command `npm ci && npx tsc --build && npm run build --workspace @effort/web`, Output Directory `apps/web/dist`, ruta `/`. Antes de crearlo se sacó `VITE_API_URL` del spec: estaba fijado a `effort360.disaak.com`, un dominio que todavía no existe (tarea 109 pendiente) — desplegar así habría roto todas las llamadas a la API hasta que el DNS resolviera, y habría hecho falta otro rebuild cuando cambiara. `apps/web/src/api/cliente.ts` ahora usa `window.location.origin` como base en producción (mismo origen que la API, sin CORS), y sigue usando `http://localhost:3000` en desarrollo (`import.meta.env.DEV` distingue los dos casos). Verificado local antes de tocar el panel: `tsc --build` sin errores, 100/100 tests de `apps/web`. **Primera vez que se usó `Claude in Chrome`** (control directo del navegador real de Daniel, ya logueado en DigitalOcean) para verificar en vez de pedir capturas de pantalla: build y deploy seguidos en vivo, y una prueba de login real contra la app desplegada — con un usuario inexistente a propósito — mostró en la red `GET /api/v1/csrf` → 200, `POST /api/v1/acceso` → 401, y el mensaje real del servidor ("Correo o contraseña incorrectos.") renderizado en la interfaz. Interfaz, API, base de datos, CSRF y manejo de errores, los cinco funcionando juntos en producción. Costo total de la app: **$5/mes** (el componente `web` no suma nada, es estático). Quedan las tareas 109 (DNS de `effort360.disaak.com`, todavía sin confirmar dónde se administra) y 110 (HTTPS/cookies contra el dominio propio, no contra la URL temporal de DigitalOcean).
- **2026-09-10** — Al ir a sembrar los 10 usuarios reales de EFFORT se encontró un hueco que hacía inútil la parte más importante del sistema: **no existía ninguna forma de que una persona gestionara sus propias credenciales.** No había ruta de cambio de contraseña (el `edicionSchema` de `PATCH /usuarios/:id` no tiene campo de contraseña, así que ni la persona podía cambiarla ni dirección resetearla), `debeCambiarContrasena` era una bandera que nada leía, y no había alta de segundo factor — `generarSecretoTotp()` y `urlDeConfiguracionTotp()` existían sin que ninguna ruta las llamara. Peor: como `direccion` y `responsable` tienen segundo factor obligatorio, alguien de esos roles sin `secretoTotp` recibía 403 **sin que se le creara sesión**, así que no podía llegar a ningún endpoint de configuración. Círculo cerrado; por eso a `effort360` hubo que cargarle el secreto con un script. **Por qué no era un detalle:** el mismo día Daniel había pedido que el sistema "siempre diga quién hizo qué modificación". La bitácora lo hace bien (registra usuario, acción, entidad, valores antes y después, IP y fecha, y sanea lo sensible), pero si un administrador fija la contraseña de cada persona y esa persona no puede cambiarla nunca, ese administrador conoce para siempre la credencial de todos, y "Fulana modificó X" deja de ser prueba de que lo hizo Fulana. Para una consultora cuyo producto es evidencia auditable, eso vacía el registro. **Decisión de secuencia:** construir esto ANTES de sembrar al equipo. Sembrar primero significaba repartir 10 credenciales permanentes conocidas por el administrador y volver a repartirlas después — el retrabajo exacto que Daniel pidió evitar. **Cómo se resolvió, y lo que vale la pena recordar del diseño:** el login ahora emite una sesión *pendiente* en vez de un 403, y no hizo falta inventar ningún estado nuevo porque `evaluarSesion` ya devolvía `SEGUNDO_FACTOR_PENDIENTE` y el `preHandler` ya se negaba a poblar `sujeto` con eso — una garantía que ya existía, solo había que apoyarse en ella. `debeCambiarContrasena` se hace cumplir con una guarda global y una lista corta de rutas permitidas, no ruta por ruta, mismo criterio que el hook de CSRF de la tarea 39: una ruta nueva que se olvide de comprobarlo es exactamente cómo se abre un agujero sin que nadie lo note. El secreto TOTP no se puede regenerar si ya hay uno (409, y la condición vive en la propia consulta, no solo en la ruta): regenerarlo con la sesión de la víctima es el ataque que el segundo factor debería frenar. Y el cambio de contraseña cierra **todas** las sesiones, incluida la propia — dejar viva la actual es cómodo pero inútil: si alguien más conocía la vieja y tenía sesión abierta, el cambio tiene que echarlo. **Verificación:** 12 tests de API, 6 de interfaz, y `e2e/pruebas/primer-acceso.spec.ts`, que recorre el camino entero en navegador real contra servidor y base reales y **lee el secreto de la pantalla en vez de hardcodearlo**, así comprueba que lo que se le muestra a la persona sirve de verdad. Ese e2e encontró de paso que `getByLabel('Contraseña nueva')` también coincidía con "Repetí la contraseña nueva" — un fallo del test, no del producto, pero es justo el tipo de cosa que solo aparece con el navegador de verdad. Se reescribió además el test que afirmaba el 403 viejo: ahora comprueba lo que de verdad importa, que la sesión pendiente no habilite ninguna ruta de negocio.
- **2026-09-10** — Sembrado el equipo real completo de EFFORT en producción: 11 usuarios (`effort360` ya existente + 10 nuevos). Roles según lo que Daniel confirmó de Laura y Lili — *"todos pueden acceder a todo, PERO siempre tiene que decir quien hizo que modificiaciòn"* y *"solo Lili, Laura y yo... tendremos Adm global, mientras que los demàs pueden crear, modificar y ver, pero no borrar"* — que se tradujo directo a la matriz de permisos ya existente sin necesidad de tocar código: `direccion` para las dos y para Daniel, `responsable` para el resto (crea/edita/ve, nunca elimina — `eliminar` no está concedida a ningún rol del sistema), `veTodosLosClientes` en todos porque no hay restricción de cartera pedida. Sembrado directo por Prisma con `creadoPorUsuarioId` apuntando al id real de `effort360`, para que la bitácora registre correctamente quién dio de alta a cada persona incluso siendo una siembra por script. Cada cuenta queda en el mismo estado que deja el alta real (`debeCambiarContrasena: true`, `secretoTotp: null`): va a pasar por el flujo de primer acceso construido para la tarea 57 apenas entre por primera vez. Verificado contra la base real antes de entregar nada, no asumido. Credenciales entregadas por archivo aparte y borradas del disco después de enviarlas — nunca aparecieron en el chat.
- **2026-09-10** — Carga real de documentos para los 5 clientes piloto, período completo del piloto (enero-junio 2026). Daniel pidió cargar "uno por uno todos los documentos sin repetirlos" — se acotó al período confirmado del piloto (`docs/DISCREPANCIAS.md` punto 4) en vez de todo el historial de cada cliente, porque es lo que efectivamente ejercita el motor de cálculo (vencimientos, alertas, IVA), que era el objetivo original de esta tarea: comprobar que el sistema funciona de verdad, no solo que se ve bien. Solo lectura sobre el registro real de EFFORT (OneDrive de Laura); solo escritura en la carpeta propia del sistema (`/EFFORT Control 360/Entrada/`) — cada archivo se copió ahí antes de registrarse. El mecanismo de deduplicación no se inventó aparte: se replicó el que ya existía en el código real (`Evidencia.sha256` único, `Documento` único por RUC+timbrado+número con `skipDuplicates: true`), verificado reintentando después de tres cortes de red transitorios y comprobando que no se generó ningún duplicado. **Resultado: 671 documentos reales, 671 evidencias, cero huérfanos, verificado contra la base de producción** — Copesa 195, Fumipro 174, Ecoagro 153, Sipar 77, Dibec 72. Solo 5 facturas electrónicas tuvieron su identidad completa (RUC/timbrado/número/total/tasa) extraída del contenido real del PDF; el resto de los documentos clasificados como factura por su nombre no pudieron leerse con certeza (escaneos, formatos distintos) y quedaron sin esos campos en vez de inventar un monto — el sistema no tiene ninguna cifra escrita a mano, tampoco ahora. Detalle completo en `docs/BITACORA-ONEDRIVE.md`. Esto no reemplaza la conexión automática y reutilizable entre OneDrive y el sistema (sigue pendiente, `docs/DISCREPANCIAS.md` punto 6) — fue una migración real de datos de una sola vez, con un script que no quedó en el repositorio.
- **2026-09-17** — Sesión de auditoría y trabajo sobre la tarea 141. Un workflow de solo lectura (124 agentes: lectores por área, un escéptico por hallazgo, síntesis y crítica) auditó el código sin commit de la 141 y produjo `docs/PLAN-MAESTRO-2026-09-16.md`. Se corrigieron los hallazgos confirmados, cada uno en su propia rama local (sin push, con permiso pendiente de Daniel): `tarea/141` (elección de planillas ya no depende del orden de la base ni de un corte de red — candado de IVA, orden total de prioridad, mes principal por planilla, reintentos en `DriveGraph.leer`, meses inválidos rechazados), `tarea/N6` (correos apagados por defecto — orden expresa de Daniel del 2026-09-16 de no mandar ningún correo hasta que él lo autorice — con lista de destinatarios y tope explícitos), `tarea/N2` (guardia de comandos versionada en el repo, ya no bloquea búsquedas inofensivas y cubre variantes que antes pasaban), `tarea/N4` (una sola lista de modelos para los dos respaldos, comparada contra el esquema real con un test), `tarea/N1` (script de espera de despliegue con dos señales, y un script de consulta de solo lectura contra producción probado CONTRA POSTGRESQL REAL — un INSERT/UPDATE/DELETE dentro de su transacción falla porque lo rechaza PostgreSQL, no por convención). Esta misma entrada corrige además dos afirmaciones desactualizadas del propio roadmap: el conteo de "100% para mostrar" decía "5 de 9" con una tabla que solo tenía 4 ✅ (corregido a 3 de 9 verificados sin dudas, bajando además la fila 6 a pendiente de reverificar en producción), y el banner de `CLAUDE.md` seguía describiendo el incidente del 13/09 como si los datos siguieran sin reconstruir. Falta: integrar las 5 ramas en `main`, correr `npm run verify` completo sobre esa integración, la lectura de solo lectura de los Excel reales de COPESA/FUMIPRO/ECOAGRO (para confirmar la regla de "período principal" y el tope de 12 meses de arrastre contra archivos de verdad) y la autorización de Daniel para el push. Nada de esto se desplegó ni se le mandó ningún correo a nadie.
