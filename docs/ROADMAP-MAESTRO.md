...
**Avance: 96 de 114 tareas (84%).** Partes 1, 2, 3, 4A, 4B, 4C, 4D y 4E cerradas. Quedan 3 tareas bloqueadas por EFFORT (2 de carga de datos + el registro en Azure AD, este último en curso — ver `docs/DISCREPANCIAS.md` punto 6), que no frenan el código.
...
- [ ] 88. Registrar la aplicación en Azure AD — **en curso desde el 2026-08-09.** Registro "EFFORT Control 360" creado, `Tenant ID`/`Client ID`/`Client Secret` cargados en `.env` y verificados con una llamada real a Microsoft (token Bearer obtenido con éxito). Permisos de aplicación agregados (`Files.ReadWrite.All`, `Mail.Send`), pero **falta el consentimiento de administrador**. **Bloqueo levantado el 2026-09-03:** `effort360@effort.com.py` es la cuenta de Daniel y ya figura como Administrador Global (confirmado por captura de Entra ID) — puede completar el consentimiento él mismo, ya no depende de que Laura o Lili hagan el clic. Falta solo que Daniel entre y lo haga: Registros de aplicaciones → EFFORT Control 360 → Permisos de API → "Conceder consentimiento de administrador". Detalle completo en `docs/DISCREPANCIAS.md`, punto 6.
