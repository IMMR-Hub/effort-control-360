...
- **2026-07-30** — Orden real de la Parte 7: la tarea 101 ("reemplazar datos-semilla por llamadas reales") es la primera sin marcar después de la 100, pero no se puede hacer todavía de una forma verificable — sin pantalla de login (tarea 102) no hay manera de conseguir una sesión real contra la cual probar que las llamadas a la API funcionan. Construir 101 antes sería trabajo que la 102 obligaría a retocar. Se invierte el orden (102 antes que 101): no es saltear una tarea imposible como la del correo, es una dependencia real entre dos tareas posibles, y el objetivo es no rehacer trabajo.
