...
**Avance: 90 de 113 tareas (80%).** Partes 1, 2, 3, 4A, 4B, 4C, 4D y 4E cerradas. Quedan 3 tareas bloqueadas por EFFORT (2 de carga de datos + el registro en Azure AD), que no frenan el código.
...
- [ ] 94. Idempotencia verificada: importar el mismo archivo dos veces no duplica. **Alcance real tras la tarea 93:** SIGA ya está resuelto desde la tarea 74 (verificado con test). Falta solo `documento`: agregar restricción única en el modelo Prisma por (`clienteId`, `rucEmisor`, `timbrado`, `numeroComprobante`) — con cuidado de no romper documentos sin esa terna, que hoy son válidos y no deben chocar entre sí — y cambiar `RepositorioDeDocumentos` de altas de a una a un método de alta en lote con `skipDuplicates: true`, mismo patrón que `RepositorioDeExportacionesSiga`. Esto también cierra de paso la falta de atomicidad encontrada en la tarea 93 (una importación grande que fallaba a mitad de camino dejaba filas sueltas): al pasar a un solo `createMany` transaccional, ambos problemas se resuelven con el mismo cambio.
